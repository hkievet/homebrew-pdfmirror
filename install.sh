...
INPUT_UUID=$(uuidgen | tr '[:lower:]' '[:upper:]')
OUTPUT_UUID=$(uuidgen | tr '[:lower:]' '[:upper:]')
ACTION_UUID=$(uuidgen | tr '[:lower:]' '[:upper:]')

# Create document.wflow matching the working workflow structure
...
# Find pdfmirror command (works for both Intel and Apple Silicon)
...
notify() {
  ...
  /usr/bin/osascript - "\$message" "\$sound" &lt;&lt;'NOTIFYEOF'
on run argv
  set theMessage to item 1 of argv
  set theSound to item 2 of argv
  display notification theMessage with title "PDFMirror" sound name theSound
end run
NOTIFYEOF
}
...
do
    ...
    if [ \$exit_code -eq 0 ]; then
        notify "Created: \$(basename "\$output_file")" "Glass"
    else
        notify "Error processing: \$(basename "\$f")" "Basso"
    fi
done</string>
					<key>CheckedForUserDefaultShell</key>
					<true/>
...
# Force Automator to recognize the workflow by opening and closing it
# This ensures Automator properly loads the action
osascript <<APPLESCRIPT 2>/dev/null || true
tell application "Automator"
    activate
    try
        set workflowFile to POSIX file "$WORKFLOW_PATH"
        open workflowFile
        delay 2
        quit saving no
    end try
